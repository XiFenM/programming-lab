...
├── compose.ephemeral.yaml           # v2rayA 临时配置 tmpfs
...
| 工作区 | 持久化 | 行为 |
| --- | --- | --- |
| `bind` | `persistent` | 宿主机与容器双向实时同步；uv、Cargo、编译缓存与 v2rayA 配置使用命名卷。适合日常开发 |
| `bind` | `ephemeral` | 源码仍双向同步；环境和缓存只在容器可写层，v2rayA 配置使用 tmpfs |
| `copy` | `persistent` | 构建时复制源码快照，不与宿主机同步；`/workspace`、缓存和 v2rayA 配置放入命名卷 |
| `copy` | `ephemeral` | 构建时复制源码快照，不同步、不使用命名卷；v2rayA 配置使用 tmpfs |
...
v2rayA 配置在 proxy 容器的 tmpfs 中，proxy 容器停止后便不再保留。
...
`container.sh` 使用；需要直接调用 Compose 时，必须叠加 `compose.gpu.yaml`（Compose
...
- `compose.persist.yaml` 可选地挂载 uv、Cargo、ccache、Triton、TileLang 和
  v2rayA 命名卷。
- `compose.ephemeral.yaml` 用 tmpfs 承载 v2rayA 配置，避免镜像声明的配置目录产生
  无法复用的匿名卷。
