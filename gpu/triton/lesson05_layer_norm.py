...
def layer_norm_forward_kernel(
    ...
    for b_id in tl.range(0, block_num):
        ...
        mask = offset < N
        x_data = tl.load(x_ptr + offset, mask=mask, other=0).to(tl.float32)
        mean_temp += x_data
    ...
    for b_id in tl.range(0, block_num):
        offset = b_id * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)  # 行内offset
        mask = offset < N
        x_data = tl.load(x_ptr + offset, mask=mask).to(tl.float32)
        x_data -= mean
    ...
    for b_id in tl.range(0, block_num):
        ...
        mask = offset < N
        x_data = tl.load(x_ptr + offset, mask=mask, other=0).to(tl.float32)
        w_data = tl.load(w_ptr + offset, mask=mask, other=0)
        b_data = tl.load(b_ptr + offset, mask=mask, other=0)
...
def layer_norm_forward(
    ...
    if N >= 2048:
        ...
        BLOCK_SIZE = 64
    layer_norm_forward_kernel[grid](x, w, b, mean, rstd, output, N, BLOCK_SIZE, eps)
...
def layer_norm_backward_first_phase(
    ...
    for bid in tl.range(0, block_num):
        ...
        x = tl.load(x_ptr + offset, mask=mask, other=mean)
        rstd = tl.load(rstd_ptr)
        x_hat = (x - mean) * rstd
        partial_dw = tl.load(partial_dw_ptr + offset, mask=mask, other=0.0)
        partial_db = tl.load(partial_db_ptr + offset, mask=mask, other=0.0)
        partial_dw += dy * x_hat
        partial_db += dy
        tl.store(partial_dw_ptr + offset, partial_dw, mask=mask)
        tl.store(partial_db_ptr + offset, partial_db, mask=mask)
    tl.debug_barrier()
    ...
    for bid in tl.range(0, block_num):
        ...
        mean = tl.load(mean_ptr)
        x = tl.load(x_ptr + offset, mask=mask, other=mean).to(tl.float32)
        rstd = tl.load(rstd_ptr)
        g = dy * w
        x_hat = (x - mean) * rstd
        gx_hat = g * x_hat
    ...
    for bid in tl.range(0, block_num):
        ...
        tl.store(dx_ptr + offset, res, mask=mask)
...
def layer_norm_backward_second_phase(
    ...
    if GROUP_SIZE <= BLOCK_SIZE:
        ...
        partial_dw = tl.load(partial_dw_ptr + offset * N, mask=mask, other=0.0)
        partial_db = tl.load(partial_db_ptr + offset * N, mask=mask, other=0.0)
...
def layer_norm_backward(
    dy: torch.Tensor, x: torch.Tensor, w: torch.Tensor, mean: torch.Tensor, rstd: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    # check inputs
    if not (
        x.is_contiguous()
    ...
    layer_norm_backward_first_phase[grid](
        dy, x, w, mean, rstd, partial_dw, partial_db, dx, lock, N, BLOCK_SIZE, GROUP_SIZE
    )
    BLOCK_SIZE = triton.next_power_of_2(GROUP_SIZE)
    grid = (N,)
    layer_norm_backward_second_phase[grid](
        partial_dw, partial_db, dw, db, N, BLOCK_SIZE=BLOCK_SIZE, GROUP_SIZE=GROUP_SIZE
    )
    return dx, dw, db
