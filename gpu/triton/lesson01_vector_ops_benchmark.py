...
def measure_axpby(
    ...
    alpha = 1.234
    beta = 2.345
    p50_ms, p20_ms, p80_ms = 0.0, 0.0, 0.0
    if provider == 'torch':
    ...
    if not (p20_ms <= p50_ms and p50_ms <= p80_ms):
        raise RuntimeError("p20 should be less than p50 and p50 should be less then p80")
    ...
    if not (gbps_lower <= gbps_p50 and gbps_p50 <= gbps_upper):
        raise RuntimeError("p80 gbps should be less than p50 gbps " +
                        "and p50 gbps should be less then p20 gbps")
...
def run_benchmark(benchmark_fn, results_path_str: str):
    ...
    with open(
        ...
        writer = csv.DictWriter(
            f,
            fieldnames=asdict(benchmark_records[0]).keys()
        )
        writer.writeheader()
...
if __name__ == "__main__":
    ...
    run_benchmark(
        ...
        os.path.join(benchmark_results_dir,"axpby_block_size")
    )
    run_benchmark(
        benchmark_axpby_tail_block,
        os.path.join(benchmark_results_dir,"axpby_tail_block")
    )
