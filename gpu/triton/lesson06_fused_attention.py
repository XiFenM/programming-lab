...
def attention_fwd_inner_loop(
    ...
    for k_block_start_loc in tl.range(start_loc, end_loc, BLOCK_N):
        ...
        row_sum = alpha * row_sum + tl.sum(score, 1)
...
def attention_fwd_kernel(
    ...
    if causal:
        acc, row_sum, score_max = attention_fwd_inner_loop(
            q_tile,
            k,
            v,
            score_max,
    ...
    score_max += tl.math.log2(row_sum)
...
def attention_forward(
    ...
    BLOCK_M = 32
    BLOCK_N = 16
    desc_q = TensorDescriptor(q, shape=[combined_dim0, D], strides=[D, 1], block_shape=[BLOCK_M, D])
    desc_k = TensorDescriptor(k, shape=[combined_dim0, D], strides=[D, 1], block_shape=[BLOCK_N, D])
    desc_v = TensorDescriptor(v, shape=[combined_dim0, D], strides=[D, 1], block_shape=[BLOCK_N, D])
    desc_o = TensorDescriptor(
        output, shape=[combined_dim0, D], strides=[D, 1], block_shape=[BLOCK_M, D]
    )
    desc_temp_M = TensorDescriptor(
        temp_M, shape=[combined_dim0], strides=[1], block_shape=[BLOCK_M]
    )
    grid = (triton.cdiv(N, BLOCK_M), B * H)
    attention_fwd_kernel[grid](
        desc_q, desc_k, desc_v, desc_o, desc_temp_M, sm_scale, causal, B, H, N, D, BLOCK_M, BLOCK_N
    )
