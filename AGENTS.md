...
`.agent-skills/`. `.agent-skills.json` selects repository Skills; `.agents/skills/` is generated by
the central materializer and must not be edited by hand.
...
tests under `tests/python/` by default. Run colocated Triton
